...
- **작업코드 표기**: 글자 하나(`A`~`H`, `J`)를 씁니다. 팀장 「작업 기준 설계」
  표는 `A-3`·`B-7` 형식이지만 뒤 숫자가 인원·시간·위험도 어느 것과도 무관한
  임의값이라, **내용은 표 그대로 두고 표기만 줄였습니다**(팀 결정 2026-09-01).
  대조표는 `scripts/seed-data.mjs` 의 `workCodes` 주석 참고. `I` 는 숫자 1 로
  읽혀 건너뜁니다
- **G-2 확인 대기**: 표에 도장부스 필수 PPE 로 안전모가 없습니다(다른 작업과
  달라 눈에 띔). 자격도 "관련 안전교육 이수"로만 적혀 있어 정식 명칭이 필요합니다
