    ...
    <div className={styles.wrap} role="table" aria-label={label}>
      ...
      {rows.length === 0 ? (
        ...
        rows.map((row) => {
          ...
          // 행 클릭이 곧 선택인 화면(W2 작업코드, W3 승인함, W5 이력)이 있습니다.
          // 행을 button 으로 내면 W3 "처리" 열의 승인/반려 버튼이 버튼 안에
          // 중첩돼 유효하지 않은 마크업이 되므로, div 에 tabIndex 와 키 핸들러를
          // 달아 키보드 동작만 button 과 맞춥니다.
          return onRowClick && !muted ? (
            <div
              key={rowKey(row)}
              role="row"
              tabIndex={0}
